...
///
/// Only V1 nests its own copy of the model. It is a frozen historical record of
/// what shipped and must never be edited - the migration matches a store
/// against it. Later versions point at the live top-level type, so there is one
/// real `Competition` and no duplicate to keep in sync.
...
/// V1 -> V2 is lightweight: one added optional property, nothing to transform.
/// Verified against a store written by the un-versioned shipped shape - the
/// rows survive and the new column is writable.
